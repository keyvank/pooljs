...
class WorkerProtocol(WebSocketServerProtocol):
	...
	def onOpen(self):
		...
		yield from worker_exists.acquire()
...
class CommanderProtocol(WebSocketServerProtocol):
	...
	def onMessage(self, payload, isBinary):
		...
		if msg["type"] == "run":
			...
			yield from job_queue.put(job_counter)
			job_counter += 1
		elif msg["type"] == "for":
...
def balance_handler():
	while True:
		job = yield from job_queue.get()
		yield from worker_exists.acquire()
		yield from worker_exists.wait_for(lambda:len(worker_websockets) > 0)
		worker_exists.release()
